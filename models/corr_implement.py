...
#代码中主要的操作是将输入张量 input 中的数据根据某种规则重新排列并存储到输出张量 output 中。这里的重新排列涉及到将二维空间中的数据进行某种形式的扩展，并且为每个元素添加一定的填充（padding）。
#在这段代码中，SIZE_1, SIZE_2, 和 SIZE_3 是用于获取张量不同维度大小的宏或函数。它们分别代表输入张量在不同维度上的大小。SIZE_1(input) channels，即通道数。
#  SIZE_1(input) 返回 channels，即通道数。
#  SIZE_2(input) 返回 height，即高度。
#  SIZE_3(input) 返回 width，即宽度。
...
'''
#这个内核函数的主要用途是计算两个特征图之间的局部相关性，
#这在计算机视觉中非常常见。通过在 rbot0
#上定义的一个小窗口区域与 rbot1 的不同偏移位置进
# 行匹配，可以得到一个表示相关性的张量 top
...
def cupy_kernel(strFunction, objVariables):
    strKernel = globals()[strFunction] #等于strFunction代表的字符串

    while True:
        objMatch = re.search('(SIZE_)([0-4])(\()([^\)]*)(\))', strKernel) # objMatch = SIZE_1(input)之类的

        if objMatch is None:
            break
        # end

        intArg = int(objMatch.group(2)) #intArg 表示正则表达式中匹配到的 SIZE_ 后面的数字

        strTensor = objMatch.group(4) #例如 匹配 SIZE_1(input) 时，group(4) 会匹配到 input 这个张量名称。
        intSizes = objVariables[strTensor].size() #返回strTensor代表的标量的维度
        strKernel = strKernel.replace(objMatch.group(), str(intSizes[intArg] if torch.is_tensor(intSizes[intArg]) == False else intSizes[intArg].item())) #返回对应的值，即将objMatch的返回值替换为intSizes[intArg]（在这里即维度信息）
