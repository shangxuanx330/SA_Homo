...
save_path="results/gfnet_dronevehicle/rumetime"
echo $save_path
script_dir=$(cd "$(dirname "$0")" && pwd)
script_path="$script_dir/$(basename "$0")"
SCEM_model=ckpts/dronevehicle/scale_similar/scem.pt
IHERM_model=ckpts/dronevehicle/scale_similar/iherm.pt
...
python inference/SA_Homo_visualization.py \
  ...
  --att_activation "gelu" \
  --kernel_fn "elu" \
  --dataset_type "gfnet_dronevehicle" \
  --attn_type MLA \
